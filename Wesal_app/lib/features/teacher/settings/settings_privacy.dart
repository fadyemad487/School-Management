...
class _SettingsPrivacyState extends State<SettingsPrivacy> {
  ...
  Future<void> _fetchDevices() async {
    ...
    try {
      final response = await _apiClient.client.get('/teachers/mobile/devices');
  ...
  Future<void> _fetchSocialStatus() async {
    ...
    try {
      final response = await _apiClient.client.get('/teachers/mobile/social/status');
      if (response.data['success'] == true) {
        if (mounted) {
          setState(() {
  ...
  Future<void> _linkSocial(String provider, String email, String socialId) async {
    ...
    try {
      ...
      if (mounted) {
        String msg = isArabic 
            ? 'حدث خطأ أثناء ربط الحساب، قد يكون الحساب مربوطاً بمستخدم آخر'
            : 'Error linking account, it might be linked to another user';
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              msg,
  ...
  Future<void> _logoutDevice(String sessionId) async {
    ...
    try {
      final response = await _apiClient.client.post(
        '/teachers/mobile/devices/logout',
  ...
  Future<void> _logoutAllOtherDevices() async {
    final isArabic = WeCircleApp.getLocale(context).languageCode == 'ar';
    try {
      final response = await _apiClient.client.post('/teachers/mobile/devices/logout-all');
      if (response.data['success'] == true) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          if (!isCurrent)
            IconButton(
              icon: Icon(Icons.logout_rounded, color: AppColors.rose, size: 20.r),
