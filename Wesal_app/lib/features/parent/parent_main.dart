...
import 'home/parent_home.dart';
import 'children/children_screen.dart';
import 'messages/parent_messages.dart';
import 'activities/parent_activities.dart';
...
class _ParentMainState extends State<ParentMain> {
  ...
  void _setupGlobalNotificationListener() {
    _socketSubscription = SocketService().onEvent.listen((eventData) async {
      ...
      debugPrint('⚡ [ParentMain] Received Socket Event: $event');
      debugPrint('⚡ [ParentMain] Socket Data Payload: $data');
      ...
      if (event == 'notification:new' || event == 'notification:system') {
        if (mounted && data != null) {
          ...
          debugPrint('⚡ [ParentMain] Local userId from prefs: "$userId"');
          debugPrint('⚡ [ParentMain] Packet recipientId: "$recipientId"');
          ...
          if (recipientId == null || recipientId == userId) {
            debugPrint(
                '⚡ [ParentMain] MATCH SUCCESSFUL! Showing notification banner.');
            _showGlobalInAppNotification(data);
          } else {
            debugPrint(
                '⚡ [ParentMain] MATCH FAILED: recipientId "$recipientId" != local userId "$userId"');
          }
        }
  ...
  void _showGlobalInAppNotification(dynamic notif) async {
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    final navItems = [
      _NavItem(Icons.home_rounded, Icons.home_outlined, 'الرئيسية', 'Home'),
      _NavItem(Icons.people_rounded, Icons.people_outline_rounded, 'أطفالي',
          'My Children'),
      _NavItem(Icons.chat_bubble_rounded, Icons.chat_bubble_outline_rounded,
          'المحادثات', 'Chats'),
      _NavItem(
          Icons.event_rounded, Icons.event_outlined, 'الفعاليات', 'Events'),
    ];
    ...
    return Scaffold(
      ...
      body: IndexedStack(
        ...
        children: [
          ParentHome(),
          ChildrenScreen(),
          ParentMessages(onConversationOpened: _clearUnreadCount),
          ParentActivities(),
        ],
      ),
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border:
                Border.all(color: widget.color.withOpacity(0.3), width: 1.5.w),
            boxShadow: [
              BoxShadow(
